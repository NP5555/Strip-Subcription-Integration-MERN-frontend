                    ...
                    <div key={customer.customerId} className="col-md-4 mb-3">
                        <div className={`card text-white ${getRandomColor()}`}>
                            <div className="card-body shadow-lg">
                                <h2 className="card-title text-warning mb-2">{customer.customerName}</h2>
                                <ul className="list-unstyled">
                                    {customer.plans.length > 0 ? (
                                        customer.plans.map(plan => (
                                            <li key={plan.planId}>
                                                <p><strong>Amount:</strong> {plan.amount} {plan.currency}</p>
                                                <p><strong>Interval:</strong> {plan.interval}</p>
                                                <p><strong>Status:</strong> {plan.status}</p>
                                                <p><strong>Start Date:</strong> {new Date(plan.start_date).toLocaleDateString()}</p>
                                                <p><strong>End Date:</strong> {new Date(plan.current_period_end).toLocaleDateString()}</p>
                                            </li>
                                        ))
                                    ) : (
